...
const Alert = (props) => {
  ...
  const body = <div className={String(alertType)}>
    <span className="alert-close" onClick={() => { setVisible(false); }} >
      &times;
    </span>
  ...
  return visible ? (<div className="fadeIn">{ body }</div>) : (<div className="fadeOut">{ body }</div>);
};
