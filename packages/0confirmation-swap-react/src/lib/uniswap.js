...
export default async function setupTestUniswapSDK(provider, getContracts) {
  ...
  Pair.getAddress = (tokenA, tokenB) => {
    ...
    return ethers.utils.getCreate2Address(getContracts().factory, ethers.utils.arrayify(ethers.utils.solidityKeccak256(["address", "address"], [tokens[0].address, tokens[1].address])), ethers.utils.arrayify(INIT_CODE_HASH));
  };
}
