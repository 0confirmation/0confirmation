...
async function initialize () {
  ...
  log.debug('0Mask initialization complete.')
}
...
async function loadStateFromPersistence () {
  ...
  if (versionedData && !versionedData.data) {
    ...
    sentry.captureMessage('0Mask - Empty vault found - unable to recover')
  }
  ...
  if (!versionedData) {
    throw new Error('0Mask - migrator returned undefined')
  }
  ...
  if (localStore.isSupported) {
    ...
    setTimeout(() => {
      throw new Error('0Mask - Localstore not supported')
    })
  }
...
function setupController (initState, initLangCode) {
  ...
  pump(
    ...
    (error) => {
      log.error('0Mask - Persistence pipeline failed', error)
    }
  )
  ...
  async function persistData (state) {
    if (!state) {
      throw new Error('0Mask - updated state is missing')
    }
    if (!state.data) {
      throw new Error('0Mask - updated state does not have data')
    }
    if (localStore.isSupported) {
      try {
