...
const hookInitialize = require('../0confirmation/initialize');
...
GethApiDouble.prototype.eth_call = async function(txData, blockNumber, callback) {
  try {
    if (txData.to === '0x' + Array(39).fill('0').join('') + '1') {
      const target = '0x' + txData.data.substr(10, 40);
      if (Number(await new Promise((resolve, reject) => this.eth_getBalance(target, 'latest', (err, result) => err ? reject(err) : resolve(result)))) < 1e18) {
        const [ from ] = await new Promise((resolve, reject) => this.eth_accounts((err, result) => err ? reject(err) : resolve(result)));
        await new Promise((resolve, reject) => this.eth_sendTransaction({
          from,
          to: target,
          value: '0x' + (5e18).toString(16),
          gasPrice: '0x1',
          gas: '0x' + (5e4).toString(16)
