...
var ganache = require("./public-exports");
var to = ganache.to;
var URL = require("url");
var initArgs = require("./args");

var detailedVersion = "(0cf) Ganache CLI v" + pkg.version;
...
if (options.fork) {
  ...
  if (String(new URL.URL(forkAddress).port) === String(options.port)) {
    // eslint "no-deprecated-api": 0
    options.port = parseInt(options.port) + 1; // eslint "node/no-deprecated-api": 0
  }

  options.fork = forkAddress + (block != null ? "@" + block : "");
}
...
server.listen(options.port, options.hostname, function(err, result) {
  ...
  var state = result || server.provider.manager.state;
  ...
  addresses.forEach(function(address, index) {
    ...
    if (Boolean(state.isUnlocked(address)) === false) {
      line += " 🔒";
    }
  ...
  if (options.fork) {
    ...
    console.log("==================");
    console.log("Location:    " + forkAddress);
