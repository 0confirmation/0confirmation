...
module.exports = async (buidler) => {
  ...
  const { log, deploy, saveDeployments } = deployments;
  const { ethers } = bre;
  const deployed = [];
  ...
  const shifterBorrowProxyFactoryLib = wrap(
    push(
      await deploy("ShifterBorrowProxyFactoryLib", {
        from: deployer,
  ...
  const shifterPool = wrap(
    push(
      await deploy("ShifterPool", {
        from: deployer,
        contractName: "ShifterPool",
        args: [],
        libraries: {
          ["ShifterBorrowProxyFactoryLib"]:
            shifterBorrowProxyFactoryLib.address,
        },
      })
    )
  );
  const erc20Adapter = wrap(
    push(
      await deploy("ERC20Adapter", {
        from: deployer,
  ...
  switch (chain) {
    ...
    case "test":
      ...
      shifterRegistry = wrap(
        push(
          await deploy("ShifterRegistryMock", {
            from: deployer,
      ...
      router = wrap(
        push(
          await deploy("UniswapV2Router01", {
            from: deployer,
  ...
  const simpleBurnLiquidationModule = wrap(
    push(
      await deploy("SimpleBurnLiquidationModule", {
        from: deployer,
  ...
  const liquidityToken = wrap(
    push(
      await deploy("LiquidityToken", {
        from: deployer,
