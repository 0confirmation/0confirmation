  ...
  NETWORK === "buidler"
    ? Object.assign(
        new ethers.providers.Web3Provider(
          fromSecret(PRIVATE_KEY, ethersToWeb3(provider))
        ).getSigner(),
        {startWatching() {}, on() {}}
      )
...
const shifterRegistry = new ethers.Contract(
  ...
  */
  ...
  if (NETWORK === "buidler") {
    zero.getRenBTC = async function () {
      ...
      return (_renbtc = await shifterRegistry
        .attach(ShifterRegistryMock.address)
        .token());
    };
  } else {
    zero.getRenBTC = async function () {
