...
library RenVMShiftMessageLib {
  ...
  function computeBorrowerSalt(RenVMShiftMessage memory message, address token, address owner) internal pure returns (bytes32) {
    return keccak256(abi.encodePacked(token, owner, message.pHash, message.amount, message.nHash));
  }
